                ...
                for (let j = 0; j < res.rows[i].productDetail.length; j++) {
                    res.rows[i].productDetail[j].productDetailSize =
                        await db.ProductDetailSize.findAll({
                            where: {
                                productdetailId:
                                    res.rows[i].productDetail[j].id,
                            },
                            raw: true,
                        });

                    res.rows[i].price =
                        res.rows[i].productDetail[0].discountPrice;
                    ...
                    for (
                        let k = 0;
                        k <
                        res.rows[i].productDetail[j].productImage.length >
                        0;
                        k++
                    ) {
                        res.rows[i].productDetail[j].productImage[k].image = res
                            .rows[i].productDetail[j].productImage[k].image
                            ? Buffer.from(
                                  res.rows[i].productDetail[j].productImage[k]
                                      .image,
                                  "base64"
                              ).toString("binary")
                            : null;
        ...
        try {
            if (!id) {
                ...
                for (let i = 0; i < res.productDetail.length > 0; i++) {
                    res.productDetail[i].productImage =
                        await db.ProductImage.findAll({
                    ...
                    for (
                        let j = 0;
                        j < res.productDetail[i].productImage.length;
                        j++
                    ) {
                        res.productDetail[i].productImage[j].image = res
                            .productDetail[i].productImage[j].image
                            ? Buffer.from(
                    ...
                    for (
                        ...
                        let receiptDetail = await db.ReceiptDetail.findAll({
                            where: {
                                productDetailSizeId:
                                    res.productDetail[i].productDetailSize[k]
                                        .id,
                            },
                        });
                        ...
                        res.productDetail[i].productDetailSize[k].stock =
                            quantity;
                    }
                }
                resolve({
                ...
                if (productdetail.rows && productdetail.rows.length > 0) {
                    for (let i = 0; i < productdetail.rows.length; i++) {
                        productdetail.rows[i].productImageData =
                            await db.ProductImage.findAll({
                                where: {
                                    productdetailId: productdetail.rows[i].id,
                                },
                            });
                        ...
                        if (
                            ...
                            for (
                                let j = 0;
                                j <
                                productdetail.rows[i].productImageData.length >
                                0;
                                j++
                            ) {
                                productdetail.rows[i].productImageData[
                                    j
                                ].image = productdetail.rows[i]
                                    .productImageData[j].image
                                    ? Buffer.from(
                                          productdetail.rows[i]
                                              .productImageData[j].image,
                                          "base64"
                                      ).toString("binary")
                                    : null;
        ...
        try {
            if (!data.id || !data.limit || !data.offset) {
                ...
                if (productImage.rows && productImage.rows.length > 0) {
                    productImage.rows.forEach((item) => {
                        if (item.image) {
        ...
        try {
            if (!id) {
                ...
                if (productdetailImage) {
                    productdetailImage.image = productdetailImage.image
                        ? Buffer.from(
            ...
            for (let i = 0; i < res.length; i++) {
                ...
                for (let j = 0; j < res[i].productDetail.length; j++) {
                    res[i].productDetail[j].productDetailSize =
                        await db.ProductDetailSize.findAll({
                            where: {
                                productdetailId: res[i].productDetail[j].id,
                            },
                            raw: true,
                        });

                    res[i].price = res[i].productDetail[0].discountPrice;
                    res[i].productDetail[j].productImage =
                        await db.ProductImage.findAll({
                            where: {
                                productdetailId: res[i].productDetail[j].id,
                            },
                    ...
                    for (
                        ...
                        res[i].productDetail[j].productImage[k].image = res[i]
                            .productDetail[j].productImage[k].image
                            ? Buffer.from(
            ...
            if (!data.userId && !data.limit) {
                ...
                if (productArr && productArr.length > 0) {
                    for (let g = 0; g < productArr.length; g++) {
                        ...
                        productArr[g].productDetail =
                            await db.ProductDetail.findAll(
                                objectFilterProductDetail
                            );

                        for (
                            let j = 0;
                            j < productArr[g].productDetail.length;
                            j++
                        ) {
                            productArr[g].productDetail[j].productDetailSize =
                                await db.ProductDetailSize.findAll({
                                    where: {
                                        productdetailId:
                                            productArr[g].productDetail[j].id,
                                    },
                                    raw: true,
                                });
                            ...
                            for (
                                ...
                                productArr[g].productDetail[j].productImage[
                                    k
                                ].image = productArr[g].productDetail[j]
                                    .productImage[k].image
                                    ? Buffer.from(
                                          productArr[g].productDetail[j]
                                              .productImage[k].image,
                                          "base64"
                                      ).toString("binary")
                                    : null;
                    ...
                    for (let g = 0; g < productArr.length; g++) {
                        ...
                        for (
                            ...
                            for (
                                ...
                                productArr[g].productDetail[j].productImage[k].image = productArr[g].productDetail[j]
                                    .productImage[k].image
                                    ? Buffer.from(
                                          productArr[g].productDetail[j]
                                              .productImage[k].image,
