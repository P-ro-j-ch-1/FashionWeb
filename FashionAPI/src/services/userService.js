...
let handleCreateNewUser = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // Validate độ dài firstName + lastName
            let fullNameLength = (data.firstName + data.lastName).trim().length;
            if (fullNameLength < 6) {
                return resolve({
                    errCode: 3,
                    errMessage:
                        "Full name (firstName + lastName) must be at least 6 characters long!",
                });
            }
                ...
                if (isExist === true) {
                    ...
                    if (user) {
                        ...
                        if (check) {
                            ...
                            userData.user = user;
                            userData.accessToken = CommonUtils.encodeToken(
                                user.id
                            );
                        } else {
                            userData.errCode = 3;
        ...
        try {
            if (!data.id || !data.password || !data.oldpassword) {
                ...
                if (await bcrypt.compareSync(data.oldpassword, user.password)) {
                    if (user) {
                        user.password = await hashUserPasswordFromBcrypt(
                            data.password
                        );
                        await user.save();
                    }
